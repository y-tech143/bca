/* =========================================================================
   🔥 LIVE DATA LAYER — backed by Firebase Firestore
   ─────────────────────────────────────────────────────────────────────────
   Nothing here is hardcoded anymore. ADMIN_NOTICES, IMPORTANT_LINKS, and
   ACADEMIC_DATABASE are now populated in real time from Firestore, and are
   rebuilt automatically the instant an admin adds/edits/deletes something
   in admin.html — no code changes, no redeploys, no page reload needed.

   Firestore collections used:
     notices        { title, date, tag, important, createdAt }
     importantLinks { title, url, createdAt }
     subjects       { semester (1-6), name, icon, order }
     resources      { subjectId, category, title, url, createdAt }
                     — category is one of: notes, previousYearPapers, pdfs,
                       guessPapers, assignments, practicals, videoLinks
                     — subjectId is either a real subject doc's ID, or the
                       special value "SYLLABUS_<semester>" for a semester's
                       syllabus links (which aren't tied to one subject).

   A semester with zero subjects, or a subject with zero resources, is
   automatically shown as "Coming Soon" — there's no separate status flag
   for the admin to manage.
   ========================================================================= */
let ADMIN_NOTICES = [];
let IMPORTANT_LINKS = [];
let ACADEMIC_DATABASE = [];
let SUBJECTS_RAW = [];
let RESOURCES_RAW = [];
let firestoreDataReady = { subjects: false, resources: false };

// Rebuilds ACADEMIC_DATABASE from the latest SUBJECTS_RAW/RESOURCES_RAW,
// in the exact same shape the rest of the app already expects — so
// renderHomeDashboard(), openSemesterView(), search, bookmarks, progress
// tracking etc. all keep working completely unchanged.
function buildAcademicDatabase() {
    const RESOURCE_KEYS = ["notes", "previousYearPapers", "pdfs", "guessPapers", "assignments", "practicals", "videoLinks"];
    const semesters = [];

    for (let s = 1; s <= 6; s++) {
        const subjectsForSem = SUBJECTS_RAW.filter(sub => sub.semester === s).sort((a, b) => (a.order || 0) - (b.order || 0));
        const syllabusItems = RESOURCES_RAW.filter(r => r.subjectId === `SYLLABUS_${s}`).map(r => ({ title: r.title, url: r.url, _id: r.id }));

        const subjects = subjectsForSem.map(sub => {
            const subResources = RESOURCES_RAW.filter(r => r.subjectId === sub.id);
            const byCategory = (cat) => subResources.filter(r => r.category === cat).map(r => ({ title: r.title, url: r.url, _id: r.id }));
            const subject = { _id: sub.id, name: sub.name, icon: sub.icon || "fa-book" };
            RESOURCE_KEYS.forEach(k => subject[k] = byCategory(k));
            if (subResources.length === 0) subject.status = "under-development";
            return subject;
        });

        semesters.push({
            semesterName: `BCA Semester ${s}`,
            status: subjects.length === 0 ? "under-development" : undefined,
            semesterSyllabus: syllabusItems.length ? syllabusItems : null,
            subjects
        });
    }
    ACADEMIC_DATABASE = semesters;
}

// Tracks whatever the student is currently looking at, so a live data update
// (someone else adding a note right now) can re-render the same view instead
// of yanking the user back to the home screen.
let currentView = { type: "home" };
function refreshCurrentView() {
    if (currentView.type === "semester") openSemesterView(currentView.semIndex);
    else if (currentView.type === "subject") openSubjectCategories(currentView.semIndex, currentView.subIndex);
    else renderHomeDashboard();
}

function handleFirestoreError(err) {
    console.error("Firestore error:", err);
    showToast("Couldn't sync live data — check your connection.", "fa-triangle-exclamation");
}

// Subscribes to every collection and keeps the whole site in sync in real time.
function setupFirestoreListeners() {
    ...
    db.collection("notices").orderBy("createdAt", "desc").onSnapshot(snap => {
        ADMIN_NOTICES = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        buildTicker();
        if (document.getElementById("noticesContainer")) renderLiveNotices();
    }, handleFirestoreError);

    db.collection("importantLinks").orderBy("createdAt", "asc").onSnapshot(snap => {
        IMPORTANT_LINKS = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        if (document.getElementById("linksContainer")) renderImportantLinks();
    }, handleFirestoreError);

    db.collection("subjects").onSnapshot(snap => {
        SUBJECTS_RAW = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        firestoreDataReady.subjects = true;
        buildAcademicDatabase();
        refreshCurrentView();
    }, handleFirestoreError);

    db.collection("resources").onSnapshot(snap => {
        RESOURCES_RAW = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        firestoreDataReady.resources = true;
        buildAcademicDatabase();
        refreshCurrentView();
    }, handleFirestoreError);
}

// Shown once, in place of the semester grid, if firebase-config.js still has
// placeholder values — so the site fails helpfully instead of just looking empty.
function showFirebaseSetupBanner() {
    const grid = document.getElementById("semestersGrid");
    if (!grid) return;
    grid.innerHTML = `
        <div class="glass" style="grid-column: 1/-1; padding: 2.5rem; text-align:center; border-radius: var(--r-lg); border-left: 4px solid var(--accent-a);">
            <i class="fa-solid fa-plug-circle-exclamation" style="font-size: 2.2rem; color: var(--accent-a); margin-bottom: 1rem;"></i>
            <h3 style="font-family: var(--font-display); margin-bottom: 0.5rem;">Firebase isn't connected yet</h3>
            <p style="color: var(--text-dim); max-width: 480px; margin: 0 auto;">
                Open <code>firebase-config.js</code> and paste in your Firebase project's config — see SETUP_GUIDE.md for the exact steps. Once that's done, this site updates live from your Admin Panel.
            </p>
        </div>
    `;
}
...
/* =========================================================================
   👥 TEAM DATA
   Names are intentionally left out of every rendered card/modal per current
   design — only role, department, and description are shown. The `name`
   field is kept as `null` here so a future admin panel can populate real
   names later without any other code changes; renderTeamCards()/openTeamProfile()
   just need one line added back (see the comment inside each) once that's wired up.
   ========================================================================= */
const TEAM_MEMBERS = [
    { name: null, gender: "male", role: "Founder", dept: "leadership", icon: "fa-crown",
        intro: "Leads the vision, growth, and future of BCA Hub.",
        bio: "Sets the direction for what BCA Hub builds next — started it as a first-year passion project so every BCA student could get organized notes for free." },
    { name: null, gender: "female", role: "Co-Founder", dept: "leadership", icon: "fa-handshake",
    ...
    { name: null, gender: "male", role: "Technical Head", dept: "tech", icon: "fa-code",
    ...
    { name: null, gender: "male", role: "Academic Head", dept: "academics", icon: "fa-book-open",
    ...
    { name: null, gender: "female", role: "Notes Manager", dept: "academics", icon: "fa-folder-open",
    ...
    { name: null, gender: "male", role: "Content Manager", dept: "content", icon: "fa-pen-nib",
    ...
    { name: null, gender: "female", role: "Graphic Designer", dept: "content", icon: "fa-palette",
    ...
    { name: null, gender: "female", role: "Social Media Manager", dept: "content", icon: "fa-hashtag",
    ...
    { name: null, gender: "female", role: "Student Support", dept: "support", icon: "fa-comments",
    ...
    { name: null, gender: "male", role: "PYQ & Resources Manager", dept: "academics", icon: "fa-file-lines",
...
// Shown as extra cards at the end of the grid — always visible, not affected by search/filter.
const TEAM_GROWTH_SLOTS = [
    { title: "We're Growing", badge: "Open Position", desc: "Join the BCA Hub Team! We're looking for passionate students to contribute to this 100% FREE learning community. Your name and profile can appear here in the future." },
    { title: "We're Growing", badge: "Open Position", desc: "Got a skill worth sharing — design, writing, coding, or coordination? There's a spot here waiting for someone like you." },
    { title: "We're Growing", badge: "Coming Soon", desc: "More roles open up every semester as BCA Hub grows. Reach out any time to find out what's currently needed." }
];
...
// A small original line-illustration bust icon — not a photo, just a clean gender-styled
// silhouette with a hinted collar, so each card has more visual identity than plain initials.
function teamAvatarSvg(gender) {
    ...
    const collar = gender === "female"
        ? `<path d="M22 40c3 5 6 7 10 7s7-2 10-7" stroke="rgba(0,0,0,0.16)" stroke-width="1.8" fill="none" stroke-linecap="round"/>`
        : `<path d="M24 39c2 5 4 7 8 7s6-2 8-7" stroke="rgba(0,0,0,0.16)" stroke-width="1.8" fill="none" stroke-linecap="round"/><path d="M28 38l4 4 4-4" stroke="rgba(0,0,0,0.12)" stroke-width="1.4" fill="none"/>`;
    return `
        <svg viewBox="0 0 64 64" class="team-avatar-svg" aria-hidden="true">
            <circle cx="32" cy="24" r="13" fill="rgba(255,255,255,0.92)"/>
            <path d="M8 58c2-14 12-22 24-22s22 8 24 22" fill="rgba(255,255,255,0.92)"/>
            ${collar}
            ${hair}
        </svg>
    `;
        ...
        <section class="hero">
            <div class="hero-grid">
                <div class="hero-content">
                    <span class="hero-badge reveal-line"><i class="fa-solid fa-graduation-cap"></i> 100% FREE Student Learning Platform</span>
                    <h1 class="hero-headline">
                        <span class="line"><span class="line-inner">Everything You Need</span></span>
                        <span class="line"><span class="line-inner">for Your <em class="gradient-text">BCA</em> Journey.</span></span>
                    </h1>
                    <p class="hero-sub">Access Notes, PYQs, Syllabus, Practical Files, Lab Programs, Previous Year Papers, and free study resources — all in one place.</p>
                ...
                <div class="hero-visual reveal-up" id="heroVisual">
                    <div class="hero-visual-glow"></div>
                    <div class="hero-dashboard-card glass" id="heroDashCard">
                        <div class="hero-dash-header">
                            <span class="hero-dash-dot red"></span><span class="hero-dash-dot yellow"></span><span class="hero-dash-dot green"></span>
                            <span class="hero-dash-title">BCA Hub Dashboard</span>
                        </div>
                        <div class="hero-dash-body">
                            <div class="hero-dash-row"><span class="hero-dash-icon"><i class="fa-solid fa-folder-open"></i></span><div class="hero-dash-lines"><span></span><span></span></div></div>
                            <div class="hero-dash-row"><span class="hero-dash-icon"><i class="fa-solid fa-file-lines"></i></span><div class="hero-dash-lines"><span></span><span></span></div></div>
                            <div class="hero-dash-row"><span class="hero-dash-icon"><i class="fa-solid fa-chart-simple"></i></span><div class="hero-dash-lines"><span></span><span></span></div></div>
                            <div class="hero-dash-progress"><div class="hero-dash-progress-fill"></div></div>
                        </div>
                    </div>

                    <div class="floating-chip chip-1" data-depth="1.4"><i class="fa-solid fa-file-pdf"></i> PDF</div>
                    <div class="floating-chip chip-2" data-depth="0.8">C++</div>
                    <div class="floating-chip chip-3" data-depth="1.1">Java</div>
                    <div class="floating-chip chip-4" data-depth="0.6">Python</div>
                    <div class="floating-chip chip-5" data-depth="1.3">HTML</div>
                    <div class="floating-chip chip-6" data-depth="0.9">CSS</div>
                    <div class="floating-chip chip-7" data-depth="1.2"><i class="fa-solid fa-database"></i> DB</div>
                    <div class="floating-chip chip-8" data-depth="0.7"><i class="fa-solid fa-robot"></i> AI</div>
                </div>
            </div>
...
function initDashboardSystem() {
    ...
    if (!firestoreDataReady.subjects) {
        semGrid.innerHTML = `
            <div class="glass" style="grid-column: 1/-1; padding: 2.5rem; text-align:center; border-radius: var(--r-lg);">
                <i class="fa-solid fa-satellite-dish" style="font-size: 1.8rem; color: var(--accent-b); margin-bottom: 0.8rem;"></i>
                <p style="color: var(--text-dim);">Connecting to live data...</p>
            </div>
        `;
        return;
    }
...
function openSemesterView(semIndex) {
    currentView = { type: "semester", semIndex };
    const context = document.getElementById("dynamicContentContext");
...
function openSubjectCategories(semIndex, subIndex) {
    currentView = { type: "subject", semIndex, subIndex };
    const context = document.getElementById("dynamicContentContext");
    const semData = ACADEMIC_DATABASE[semIndex];
    const subData = semData && semData.subjects[subIndex];

    if (!subData) {
        showToast("That subject isn't available anymore — showing the semester instead.", "fa-circle-info");
        openSemesterView(semIndex);
        return;
    }
...
document.addEventListener("keydown", (e) => { if (e.key === "Escape") { closePremiumModal(); closeTeamProfile(); if (window.closeGuideModal) window.closeGuideModal(); } });
...
function renderTeamCards() {
    ...
    const realCards = TEAM_MEMBERS.map((m, i) => `
        <div class="team-card glass clickable" data-dept="${m.dept}" data-cursor-label="View" onclick="openTeamProfile(${i})">
            <div class="team-avatar">
                <div class="team-avatar-inner">${teamAvatarSvg(m.gender)}</div>
                <span class="team-avatar-badge"><i class="fa-solid ${m.icon}"></i></span>
            </div>
            <!-- Real names, once available from an admin panel, would render here as a <p class="team-member-name"> above the role heading -->
            <h3 class="team-name">${m.role}</h3>
    ...
    const growthCards = TEAM_GROWTH_SLOTS.map(slot => `
        <div class="team-card placeholder-card glass clickable" data-cursor-label="Join" onclick="handleJoinTeamClick()">
            <div class="team-avatar placeholder-avatar">
                <div class="team-avatar-inner"><i class="fa-solid fa-user-plus"></i></div>
                <span class="team-avatar-badge"><i class="fa-solid fa-star"></i></span>
            </div>
            <h3 class="team-name">${slot.title}</h3>
            <span class="team-dept-badge open-badge">${slot.badge}</span>
            <p class="team-intro">${slot.desc}</p>
            <button class="team-view-btn placeholder-btn" onclick="event.stopPropagation(); handleJoinTeamClick()">
                <i class="fa-solid fa-hand-sparkles"></i> Join Us
            </button>
        </div>
    `).join('');

    grid.innerHTML = realCards + growthCards;
...
function setupTeamSearchAndFilter() {
    ...
    function applyFilters() {
        ...
        // Only the first TEAM_MEMBERS.length cards are real/indexed — growth-slot cards
        // that follow are intentionally left untouched so they always stay visible.
        const cards = document.querySelectorAll("#teamGrid .team-card:not(.placeholder-card)");
        cards.forEach((card, i) => {
            const m = TEAM_MEMBERS[i];
            if (!m) return;
            const matchesDept = activeDept === "all" || m.dept === activeDept;
            const matchesTerm = !term || m.role.toLowerCase().includes(term) || teamDeptLabel(m.dept).toLowerCase().includes(term);
...
function openTeamProfile(idx) {
    ...
    document.getElementById("teamModalBody").innerHTML = `
        <div class="team-modal-avatar">${teamAvatarSvg(m.gender)}</div>
        <h3>${m.role}</h3>
        <span class="team-dept-badge">${teamDeptLabel(m.dept)}</span>
...
function setupTeamStatics() {
    ...
    if (joinBtn) joinBtn.addEventListener("click", handleJoinTeamClick);
}

function handleJoinTeamClick() {
    openMailto("Interested in joining the BCA Hub team", "Hi BCA Hub,\n\nI'd like to join the team and contribute. Here's a bit about me and what I can help with:\n\n");
    showToast("Opening your email app to reach BCA Hub...", "fa-hand-sparkles");
}

/* =========================================================================
   ✉️ CONTACT — every "reach out" action in the site routes through this
   ========================================================================= */
const BCA_HUB_EMAIL = "bca24by7@gmail.com";

function openMailto(subject, body) {
    const url = `mailto:${BCA_HUB_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body || "")}`;
    window.location.href = url;
}

function handleContributeNotes() {
    openMailto("I'd like to share my notes with BCA Hub", "Hi BCA Hub,\n\nI have notes/PYQs/practicals I'd like to contribute:\n\nSemester: \nSubject: \nWhat I'm sharing: \n\n(You can attach your files directly to this email.)\n");
    showToast("Opening your email app to send us your notes...", "fa-paper-plane");
}

function handleSendFeedback() {
    openMailto("Feedback for BCA Hub", "Hi BCA Hub,\n\nHere's my feedback:\n\n");
    showToast("Opening your email app to send feedback...", "fa-comment-dots");
}
...
// Subtle mouse parallax on the hero visual — the dashboard card and each floating
// chip drift slightly opposite the cursor, chips further out moving more (data-depth).
function setupHeroParallax() {
    ...
    const chips = visual.querySelectorAll(".floating-chip");
    const dashCard = document.getElementById("heroDashCard");
    ...
    hero.addEventListener("mousemove", (e) => {
        ...
        if (dashCard) dashCard.style.transform = `translate(${px * 14}px, ${py * 14}px)`;
        chips.forEach(chip => {
            const depth = parseFloat(chip.dataset.depth) || 1;
            chip.style.setProperty("--parallax-x", `${px * 22 * depth}px`);
            chip.style.setProperty("--parallax-y", `${py * 22 * depth}px`);
        });
    });
    hero.addEventListener("mouseleave", () => {
        if (dashCard) dashCard.style.transform = "";
        chips.forEach(chip => { chip.style.removeProperty("--parallax-x"); chip.style.removeProperty("--parallax-y"); });
    });
}
...
function playHeroReveal() {
    ...
    const eyebrow = document.querySelector(".hero-badge.reveal-line");
    if (window.gsap) {
...
/* =========================================================================
   📖 GUIDE MODAL — how to use the site
   Has its own EN/HI toggle, independent from the rest of the site (which
   always stays in English) — only the guide's explanations get translated.
   ========================================================================= */
const GUIDE_CONTENT = {
    en: {
        title: "How to use BCA Hub",
        intro: "Everything the site can do, explained in one place. Use the button above to read this in Hindi.",
        sections: [
            { icon: "fa-layer-group", title: "Browse by semester", desc: "Start from the home page and tap any semester card. Semesters still being filled in show a 'Coming Soon' badge, so you always know what's ready to use." },
            { icon: "fa-folder-open", title: "Open a subject", desc: "Each semester lists its subjects. Tap one to see its resources sorted into categories — Notes, Previous Year Papers, Reference PDFs, Guess Papers, Assignments, Practicals, and Video Lectures. Tap a category header to expand it." },
            { icon: "fa-eye", title: "View or Download", desc: "Every resource has two buttons: View opens it right inside the page, so you never have to leave the site. Download saves the actual file (or the closest direct link) to your device." },
            { icon: "fa-magnifying-glass", title: "Search anything", desc: "Use the search bar in the navbar, or press Ctrl+K (Cmd+K on Mac) to jump straight into it. A quick-results dropdown shows top matches as you type — press Enter to see the full results page." },
            { icon: "fa-star", title: "Save for later & track progress", desc: "Tap the ⭐ on any resource to add it to 'My Saved Resources' on the home page. Tap the ✓ to mark it as studied — this fills in a progress bar on that subject's card, so you can see how far along you are." },
            { icon: "fa-palette", title: "Change how it looks", desc: "Click the palette icon in the navbar to pick from six themes (Dark, Light, AMOLED, Blue, Green, Purple), or set any custom accent color you like. Your choice is remembered next time you visit." },
            { icon: "fa-robot", title: "Ask AI or get in touch", desc: "Use the 'Ask AI' button in the navbar for instant help with doubts. Or scroll down to the 'Get Involved' section to send us your own notes or feedback directly by email." }
        ]
    },
    hi: {
        title: "BCA Hub कैसे इस्तेमाल करें",
        intro: "साइट पर जो कुछ भी किया जा सकता है, वो सब यहाँ एक जगह समझाया गया है। ऊपर दिए बटन से इसे अंग्रेज़ी में भी पढ़ सकते हैं।",
        sections: [
            { icon: "fa-layer-group", title: "सेमेस्टर के अनुसार ब्राउज़ करें", desc: "होम पेज से शुरू करें और किसी भी सेमेस्टर कार्ड पर टैप करें। जो सेमेस्टर अभी तैयार हो रहे हैं, उन पर 'Coming Soon' बैज दिखता है, ताकि आपको हमेशा पता रहे कि क्या इस्तेमाल के लिए तैयार है।" },
            { icon: "fa-folder-open", title: "किसी सब्जेक्ट को खोलें", desc: "हर सेमेस्टर में उसके सब्जेक्ट्स लिस्ट होते हैं। किसी एक पर टैप करें और उसके संसाधन कैटेगरी में देखें — Notes, Previous Year Papers, Reference PDFs, Guess Papers, Assignments, Practicals, और Video Lectures। किसी कैटेगरी को खोलने के लिए उसके हेडर पर टैप करें।" },
            { icon: "fa-eye", title: "View करें या Download करें", desc: "हर रिसोर्स में दो बटन होते हैं: View उसे सीधे पेज के अंदर खोल देता है, ताकि आपको साइट छोड़नी ही न पड़े। Download असली फाइल (या सबसे नज़दीकी सीधा लिंक) आपके डिवाइस में सेव कर देता है।" },
            { icon: "fa-magnifying-glass", title: "कुछ भी खोजें", desc: "नेवबार के सर्च बार का इस्तेमाल करें, या Ctrl+K (Mac पर Cmd+K) दबाकर सीधे वहीं पहुंचें। टाइप करते ही एक क्विक-रिजल्ट्स ड्रॉपडाउन दिखता है — पूरा रिजल्ट्स पेज देखने के लिए Enter दबाएं।" },
            { icon: "fa-star", title: "बाद के लिए सेव करें और प्रोग्रेस ट्रैक करें", desc: "किसी भी रिसोर्स पर ⭐ टैप करें, वो होम पेज पर 'My Saved Resources' में जुड़ जाएगा। ✓ टैप करके उसे Studied मार्क करें — इससे उस सब्जेक्ट के कार्ड पर एक प्रोग्रेस बार भर जाता है, ताकि आपको पता रहे कि आप कहाँ तक पहुंचे हैं।" },
            { icon: "fa-palette", title: "लुक बदलें", desc: "नेवबार में पैलेट आइकन पर क्लिक करके छह थीम्स (Dark, Light, AMOLED, Blue, Green, Purple) में से कोई भी चुनें, या अपनी पसंद का कोई भी कस्टम कलर सेट करें। अगली बार आने पर आपकी पसंद याद रहती है।" },
            { icon: "fa-robot", title: "AI से पूछें या हमसे संपर्क करें", desc: "किसी भी doubt में तुरंत मदद के लिए नेवबार का 'Ask AI' बटन इस्तेमाल करें। या नीचे 'Get Involved' सेक्शन में जाकर हमें अपने नोट्स या फीडबैक सीधे ईमेल से भेजें।" }
        ]
    }
};
...
function setupGuideModal() {
    ...
    const openBtnMobile = document.getElementById("guideBtnMobile");
    ...
    function renderGuideBody() {
        const c = GUIDE_CONTENT[guideLang];
        const sectionsHtml = c.sections.map((s, i) => `
            <div class="guide-step">
                <span class="guide-step-num"><i class="fa-solid ${s.icon}"></i></span>
        ...
        document.getElementById("guideModalBody").innerHTML = `
            <div class="guide-modal-head">
                <div>
                    <h3>${c.title}</h3>
                    <p class="guide-intro">${c.intro}</p>
                </div>
                <button class="lang-toggle" id="guideLangToggle" type="button">
                    <i class="fa-solid fa-language"></i> <span>${guideLang === "en" ? "हिंदी में पढ़ें" : "Read in English"}</span>
                </button>
            </div>
            ${sectionsHtml}
        `;

        document.getElementById("guideLangToggle").addEventListener("click", () => {
            guideLang = guideLang === "en" ? "hi" : "en";
            renderGuideBody();
        });
    }
    ...
    if (openBtnMobile) openBtnMobile.addEventListener("click", (e) => { e.preventDefault(); document.getElementById("navMobile").classList.remove("open"); open(); });
    if (closeBtn) closeBtn.addEventListener("click", close);
    modal.addEventListener("click", (e) => { if (e.target === modal) close(); });
    window.closeGuideModal = close; // exposed so the global Escape-key handler can reach it
}
